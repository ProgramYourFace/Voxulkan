...
public:
	explicit MPMCQueue(const size_t capacity)
		...
		assert(reinterpret_cast<char*>(&tail_) - reinterpret_cast<char*>(&head_) >= static_cast<size_t>(kCacheLineSize) &&
...
private:
	...
	struct Slot {
		...
		typename std::aligned_storage<sizeof(T), alignof(T)>::type storage = { 0 };
	};
